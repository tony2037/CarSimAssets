...
namespace UnityStandardAssets.Vehicles.Car
{
    ...
    public class CarController : MonoBehaviour
    {
        ...
        public float m_Reward;  // Reward for reinforcement learning
        public int m_Emit;  // how many times that we have emmited
        public bool pause;  // is going to pause the game or not
        private int max_steps; // maximum steps that we take
        public int m_ifDone;  // to see if done
        ...
        private void Start ()
        {
            // Initialize for reinforcement learning
            InitializeActorCriticNetwork();
            //
        ...
        public void Update()
        {
            ...
            CalculateReward();
            Debug.Log("Reward: " + m_Reward);
            //if(m_Emit == )
        }

        public void CalculateReward()
        {
            // Reward function, base on distance
            if(!pause)
                m_Reward = 5f* 1f/(0.1f + Vector3.Distance(new Vector3(188.32f, -79.23f, -50.46f), transform.position));
        }
        ...
        void OnCollisionEnter(Collision collision){
            ...
            Debug.Log("Car controller: " + collider_name);
            if(collision.collider.name.Contains("Parking")){
                m_Reward = 50; // if hit the parking, give a bigger reward
                Gameover();
                Debug.Log("Gameover, success");
            }
		    else if(!collider_name.Contains("road") && collider_name != "ColliderBottom" && collision.collider.name != "Parking" || collider_name.Contains("sign")){
			    m_Reward = -100f; // collision happens, Penalty
                Gameover();
                Debug.Log("Gameover, because of crashing");
		    }
        ...
        public void InitializeActorCriticNetwork(){
            m_Reward = 0;
            m_Emit = 0;
            pause = false;
            m_ifDone = 0;
